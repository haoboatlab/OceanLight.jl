...
To setup the stage for light refraction, the data of water surface distribution is needed: water elevation $\eta$, the slope in x direction $\eta_{x} = \frac{\partial \eta}{\partial x}$ and y direction $\eta_{y} = \frac{\partial \eta}{\partial y}$. HydrOptics.jl offers random wave distribution or input in `.h5` format. 
...
```
The water surface distribution data may or may not match the input irradiance field. Hece, HydrOptics.jl interpolates the value $\eta$, $\eta_x$, and $\eta_y$, to match the corresponding irradiance field. 
